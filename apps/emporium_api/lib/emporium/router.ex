defmodule Emporium.Router do
  ...
  `Emporium.Router` imports functions from both `Plug.Conn` and `Phoenix.Controller`
  ...
  defp defs() do
    quote unquote: false do
      var!(add_route, Emporium.Router) = fn route ->
      ...
      var!(add_resources, Emporium.Router) = fn resource ->
        path = resource.path
    ...
    quote location: :keep do
      ...
      defp dispatch(conn, []) do
        try do
          ...
          kind, %{__struct__: KeyError} = reason ->
  ...
  defp add_resources(path, controller, options, do: context) do
    ...
    quote do
      ...
      var!(add_resources, Emporium.Router).(resource)
      unquote(scope)
    end
  end
